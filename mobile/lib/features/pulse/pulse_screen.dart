...
class _PulseScreenState extends State<PulseScreen> {
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    children: [
                      ...
                      if (_feed.isEmpty)
                        const Padding(padding: EdgeInsets.all(24), child: EmptyView(message: 'No pulse insights right now'))
                      else
  ...
  Widget _insightCard(Map<String, dynamic> insight) {
    final title = (insight['title'] ?? insight['headline'] ?? insight['category'] ?? 'Insight').toString();
    final body = (insight['text'] ?? insight['body'] ?? insight['description'] ?? insight['insight'] ?? '').toString();
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(title, style: GoogleFonts.spaceGrotesk(fontWeight: FontWeight.w700, fontSize: 15)),
              ),
              if (score != null)
                Container(
                  ...
                  child: Text(
                    formatNumber(score),
                    style: GoogleFonts.jetBrainsMono(color: CrickTheme.lime, fontSize: 11, fontWeight: FontWeight.w700),
                  ),
                ),
            ],
          ),
          if (body.isNotEmpty) ...[
            ...
            Text(body, style: const TextStyle(color: CrickTheme.textSecondary, height: 1.35)),
          ],
          if (insight['stats'] is Map) ...[
            const SizedBox(height: 8),
            Text(
              (insight['stats'] as Map).entries.take(4).map((e) => '${e.key}: ${e.value}').join(' · '),
              style: GoogleFonts.jetBrainsMono(fontSize: 11, color: CrickTheme.cyan),
            ),
          ],
        ],
      ),
