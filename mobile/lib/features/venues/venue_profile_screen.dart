...
class _VenueProfileScreenState extends State<VenueProfileScreen> {
  ...
  Widget build(BuildContext context) {
    final batters = asMapList(_top, 'batters').isNotEmpty ? asMapList(_top, 'batters') : asMapList(_top, 'top_batters');
    final bowlers = asMapList(_top, 'bowlers').isNotEmpty ? asMapList(_top, 'bowlers') : asMapList(_top, 'top_bowlers');
                  ...
                  children: [
                    CrickCard(
                      child: Column(
                        ...
                        children: [
                          Text(widget.venueName, style: GoogleFonts.spaceGrotesk(fontSize: 18, fontWeight: FontWeight.w700)),
                          const SizedBox(height: 8),
                          KeyValueRow('Matches', formatNumber(_stats?['matches'] ?? _stats?['total_matches'])),
                          KeyValueRow('Avg 1st inns', formatDecimal(_stats?['avg_first_innings'] ?? _stats?['avg_score'])),
                          KeyValueRow('Avg 2nd inns', formatDecimal(_stats?['avg_second_innings'])),
                          KeyValueRow('Chase win %', formatDecimal(_stats?['chase_win_pct'] ?? _stats?['chase_percentage'])),
                          KeyValueRow('Highest total', formatNumber(_stats?['highest_total'] ?? _stats?['highest'])),
                        ],
                      ),
                    ),
